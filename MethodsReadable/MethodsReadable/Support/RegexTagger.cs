...
namespace MethodsReadable.Support
{
	/// <summary>
	...
	{
		private readonly Regex[] matchExpressions;

		public RegexTagger(ITextBuffer buffer, Regex[] matchExpressions)
		{
		...
		public virtual IEnumerable<ITagSpan<T>> GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			foreach (var line in GetIntersectingLines(spans))
			{
				...
				foreach (var regex in this.matchExpressions)
				{
					...
					if (match.Success)
					{
						var tags = TryCreateTagForMatch(line.Start, match);
		...
		protected abstract IEnumerable<TagSpan<T>> TryCreateTagForMatch(SnapshotPoint lineStart, Match match);
